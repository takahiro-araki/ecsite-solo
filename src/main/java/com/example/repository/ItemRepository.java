...
public class ItemRepository {
	...
	//サーチバイメソッドで代替するのでいらない
	/**
	 * 商品を全件検索する.
	 * 
	 * @return 商品リスト
	 *//*
		 * public List<Item> findAll(){ String
		 * sql="SELECT id,name,description,price_m,price_l,deleted,image_path FROM items"
		 * ; List<Item> itemList=template.query(sql, ITEM_ROW_MAPPER); return itemList;
		 * }
		 * 
		 */
	...
	public List<Item> serchByName(String name, String order, Integer viewSize, Integer startPoint) {
		...
		sql.append(
				"SELECT id,name,description,price_m,price_l,deleted,image_path FROM items WHERE name LIKE :name ORDER BY ");
		sql.append(" " + order + " ");
